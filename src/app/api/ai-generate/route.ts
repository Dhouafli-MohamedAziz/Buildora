...
export async function POST(req: Request) {
  try {
    const { sectionName, projectDescription, theme } = await req.json();

    // Create specific prompts for each section type
    const getSectionPrompt = (section: string, description: string, theme: any) => {
      ...
      - Ne mets pas \`\`\`html ou \`\`\`
      - Donne juste le HTML pur prêt à utiliser
      `;
    ...
    const prompt = getSectionPrompt(sectionName, projectDescription, theme);
